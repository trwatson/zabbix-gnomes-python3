...
parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,description='Tries to unlink Zabbix hosts from the specified templates. Hosts can be specified seperately or by hostgroups.', epilog="""
...
group.add_argument('-H', '--hostnames' ,help='Hostname(s) to unlink template(s) from', nargs='+')
group.add_argument('-G', '--hostgroups' ,help='Unlink all the hosts in the specified hostgroup(s) from the template(s)', nargs='+')
parser.add_argument('-t', '--templates', help='Template(s) to unlink from the host(s)', nargs='+', required=True)
parser.add_argument('-N', '--noclear', help='Unlink Templates, but don\'t clear. Default is to perform an "unlink and clear"',action='store_true')
...
hostids = [i['hostid'] for i in hlookup if 'hostid' in i]
templateids = [i['templateid'] for i in tlookup if 'templateid' in i]

if args.noclear:
   try:
     # Apply the linkage
     result=zapi.host.massremove(hostids=hostids,templateids=templateids)
   except:
     sys.exit("Error: Something went wrong while performing the update")
else:
   try:
     # Apply the linkage
     result=zapi.host.massremove(hostids=hostids,templateids_clear=templateids)
   except:
     sys.exit("Error: Something went wrong while performing the update")
